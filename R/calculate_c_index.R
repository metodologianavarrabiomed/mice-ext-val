...
#'
#' @description
#' This function calculates the Harrell C Index value of a `model` in each of the imputed dataset stored in `data` as a long dataset. The Harrell's C Index calculation is realized using the `Hmisc::rcorr.cens` function [1]. After calculating the index in each imputed dataset then it aggregates them using the rubin rules with the `psfmi::pool_RR` function [2]. Finally, it stores the result inside the `c_index` attribute of the `model`.
#'
#' @references
