...
boot_data <- rsample::bootstraps(data, times = 5)
data <- purrr::map(
  boot_data$splits,
  \(x) {
    rsample::analysis(x)
...
From the data we need to look for missing values. Therefore, we will make an exploratory data analysis. 
...
The `MiceExtVal` package works with multiple imputed data and the proposed dataset does not have any missing values so we have to generate them. There are some tools that allow you to randomly generate the missing values through all the dataset, we will be using `mice::ampute` this function generates a dataset with $1 - prop$ complete cases. In our case as it only generates one missing value per row we can increase the value of `prop`up to $prop = 0.85$. We have to limit the missing values only to the possible predictor variables. Therefore, the variables `time` and `DEATH_EVENT` that form the outcome of the models will be no part of the possible missing values.
...
Once we have generated the missing values and store the dataset in `incomplete_data` we are ready to impute this missing values through the function `mice::mice`. We have to determine the number of imputed datasets that will be generated, a rule of thumb that is normally used is to generate as many imputed datasets as percentage of incomplete cases. So, in our case $m = 85$ where $m$ is the number of generated datsets.
...
It is needed to rename the variable `.id` to `id` because some of the package functions expect this variable to be part of the dataset. If you have already an `id` variable defined you can ignore this step. The package is designed to work with `survival` outcomes thus we are generating it from the beginning instead of generating it in the future. At this point we have generated the external validation dataset but we still do not have the model to be externally validated, the next section present the models that will be validated. 

# Model to validate
With the variables reported in the dataset there are no model that could be validate so instead we will generate our own model and externally validate it. We will derive two models one logistic regression and one Cox. The model derivation will be using the complete dataset and the validation will be done with `imputed_dataset`.

We will define two models with some of the `data` variables. From all the possible combinations we will try to generate a simple model with the next formula
...
# External validation
The external validation should explore the model performance in terms of calibration and discrimination. A good calibration performance represents that the model predictions are adjusted to the observed results in the external validation dataset and, equally, a good discrimination performance represents that the greater predictions from the model the greater risk that is observed in the dataset. The `MiceExtVal` package intends to generate a framework/methodology to externally validate models working with multiple imputed data.

We can divide the external validation in three steps:

1. Generate the model definition with `mv_model` functions.
2. Calculate the predictions and the c-index with the `calculate_` functions.
3. Obtain the results with the `get_` functions.


## Generation of the models 
From the previous derived models we will apply the external validation framework/methodology over the imputed dataset. We will work with the two models, the Cox model and the logistic regression one. The Cox model can be generated from `mv_model_cox` and the logistic regression from `mv_model_logreg`. These functions require different data depending on the model and therefore they have different parameters.
...
where $S_0(t)$ is the basal survival function for the time $t$, $\beta$ are the model coefficients and $X$ usually are the values in each variable centered by the mean in the derivation cohort. In the `mv_model_cox` function we have to insert the next parameters.
