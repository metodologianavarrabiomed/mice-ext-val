...
# Explore the Dataset
The first step of every external validation is to become familiar with the dataset. To perform an external validation of the Framingham model, we require a dataset that contains the relevant predictor variables used by the model. For this purpose, the `MiceExtVal` package includes a version of the Framingham dataset, originally shared on [Kaggle](https://www.kaggle.com) by [Shrey Jain](https://www.kaggle.com/datasets/shreyjain601/framingham-heart-study). Along this report we will use the dataset that it is accessible as `MiceExtVal::framingham` in the package. This dataset is the data corresponding to the third period of the original dataset with a 10-year truncated follow-up.
...
The dataset contains some missing values in cholesterol-related variables `Total Cholesterol`,and `HDL Cholesterol`. These missing values must be addressed before proceeding with model validation. @tbl-perc-miss shows the percentage of missing data for each variable, highlighting the extent of missingness across the dataset.
...
```{r}
men_model <- MiceExtVal::mv_model_cox(formula = men_formula, S0 = 0.9002)
```
...
```{r}
women_model <- MiceExtVal::mv_model_cox(formula = women_formula, S0 = 0.9625)
```
