...
#| classes: plain .table-sm
timi2_model[["predictions_agg"]] |>
  utils::head() |>
  kableExtra::kbl()

timi2_model[["predictions_imp"]] |>
  utils::head() |>
  kableExtra::kbl()
```
...
auc <- timi2_model[["results_agg"]] |> dplyr::filter(name == "auc")

auc_res <- sprintf(
  "%.3f (95%% CI %.3f, %.3f)",
  auc[["estimate"]], auc[["lower"]], auc[["upper"]]
)
```
...
* Type 1 recalibration: $\alpha_{\text{type1}} = `r round(timi2_model[["recal_parameters"]] |> dplyr::filter(param == "alpha_type_1") |> dplyr::pull("value"), 3)`$
* Type 2 recalibration: $\alpha_{\text{type2}} = `r round(timi2_model[["recal_parameters"]] |> dplyr::filter(param == "alpha_type_2") |> dplyr::pull("value"), 3)`, \beta_{\text{overall}} = `r round(timi2_model[["recal_parameters"]] |> dplyr::filter(param == "beta_overall") |> dplyr::pull("value"), 3)`$
...
timi2_model <- timi2_model |>
  MiceExtVal::calculate_brier_score(
    data = gusto_imp, type = "prediction_type_1", n_boot = 100
  ) |>
  MiceExtVal::calculate_brier_score(
    data = gusto_imp, type = "prediction_type_2", n_boot = 100
  )

brier_score_type_1 <- timi2_model[["results_agg"]] |> 
  dplyr::filter(name == "brier_score_type_1")

brier_score_type_1_res <- sprintf(
  "%.3f (95%% CI %.3f, %.3f)",
  brier_score_type_1["estimate"],
  brier_score_type_1["lower"],
  brier_score_type_1["upper"]
)

brier_score_type_2 <- timi2_model[["results_agg"]] |> 
  dplyr::filter(name == "brier_score_type_2")

brier_score_type_2_res <- sprintf(
  "%.3f (95%% CI %.3f, %.3f)",
  brier_score_type_2["estimate"],
  brier_score_type_2["lower"],
  brier_score_type_2["upper"]
)
```
...
timi2_model_men <- timi2_model_men |>
  ...
  MiceExtVal::calculate_brier_score(
    data = gusto_imp_men, type = "prediction", n_boot = 100
  ) |>
  MiceExtVal::calculate_brier_score(
    data = gusto_imp_men, type = "prediction_type_1", n_boot = 100
  ) |>
  MiceExtVal::calculate_brier_score(
    data = gusto_imp_men, type = "prediction_type_2", n_boot = 100
  )
...
timi2_model_women <- timi2_model_women |>
  ...
  MiceExtVal::calculate_brier_score(
    data = gusto_imp_women, type = "prediction", n_boot = 100
  ) |>
  MiceExtVal::calculate_brier_score(
    data = gusto_imp_women, type = "prediction_type_1", n_boot = 100
  ) |>
  MiceExtVal::calculate_brier_score(
    data = gusto_imp_women, type = "prediction_type_2", n_boot = 100
  )
```
