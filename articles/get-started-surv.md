...
and \\239\\ mg/dL, between \\240\\ y \\279\\ mg/dl, and more than
...
less than \\35\\ mg/dL, between \\35\\ y \\44\\ mg/dL, between \\45\\ y
\\49\\ mg/dL, between \\50\\ y \\59\\ mg/dL, and more than \\60\\ mg/dL.
As before the following code snippet generates the variable in the
dataset.
...
In the model it is defined a variable that aggregates the systolic blood
...
one as a representation of their state. The following code adds the
...
dplyr::bind_rows(
  men_model[["results_agg"]] |> dplyr::filter(name == "harrell_c_index"),
  women_model[["results_agg"]] |> dplyr::filter(name == "harrell_c_index")
) |>
  tibble::add_column(Model = c("Men", "Women"), .before = "estimate") |>
  kableExtra::kbl(digits = 3)
```

| name            | Model | estimate | lower | upper | p_val |
|:----------------|:------|---------:|------:|------:|------:|
| harrell_c_index | Men   |    0.685 | 0.665 | 0.705 |     0 |
| harrell_c_index | Women |    0.712 | 0.687 | 0.737 |     0 |

Table 5: C-Index results
...
is mandatory to have calculated before the model predictions or their
...
have an observed risk for each of the patients. Nevertheless we can
...
prediction for the patient \\t\\ y \\o_t\\ is the observed risk for the
patient \\t\\ in this case \\0\\ if they suffer no event and \\1\\
otherwise.
...
men_model <- men_model |>
  MiceExtVal::calculate_brier_score(
    data = men_fram_long, type = "prediction", n_boot = 100
  )

women_model <- women_model |>
  MiceExtVal::calculate_brier_score(
    data = women_fram_long, type = "prediction", n_boot = 100
  )
dplyr::bind_rows(
  men_model[["results_agg"]] |> dplyr::filter(name == "brier_score"),
  women_model[["results_agg"]] |> dplyr::filter(name == "brier_score")
) |>
  tibble::add_column(Model = c("Men", "Women"), .before = "estimate") |>
  kableExtra::kbl(digits = 3)
```
...
| name        | Model | estimate | lower | upper | p_val |
|:------------|:------|---------:|------:|------:|------:|
| brier_score | Men   |    0.255 | 0.239 | 0.270 |    \- |
| brier_score | Women |    0.183 | 0.171 | 0.195 |    \- |

Table 6: Brier score results

In [Table 6](#tbl-brier-score) there are represented the brier score
...
as shown in [Table 7](#tbl-brier-score-recal).

``` r
men_model <- men_model |>
  MiceExtVal::calculate_brier_score(
    data = men_fram_long, type = "prediction_type_1", n_boot = 100
  ) |>
  MiceExtVal::calculate_brier_score(
    data = men_fram_long, type = "prediction_type_2", n_boot = 100
  )

women_model <- women_model |>
  MiceExtVal::calculate_brier_score(
    data = women_fram_long, type = "prediction_type_1", n_boot = 100
  ) |>
  MiceExtVal::calculate_brier_score(
    data = women_fram_long, type = "prediction_type_2", n_boot = 100
  )

dplyr::bind_rows(
  men_model[["results_agg"]] |> dplyr::filter(name == "brier_score_type_1"),
  women_model[["results_agg"]] |> dplyr::filter(name == "brier_score_type_1")
  ) |>
  tibble::add_column(Model = c("Men", "Women"), .before = "estimate") |>
  kableExtra::kbl(digits = 3)
dplyr::bind_rows(
  men_model[["results_agg"]] |> dplyr::filter(name == "brier_score_type_2"), 
  women_model[["results_agg"]] |> dplyr::filter(name == "brier_score_type_2")
  ) |>
  tibble::add_column(Model = c("Men", "Women"), .before = "estimate") |>
  kableExtra::kbl(digits = 3)
```
...
| name               | Model | estimate | lower | upper | p_val |
|:-------------------|:------|---------:|------:|------:|------:|
| brier_score_type_1 | Men   |    0.254 | 0.240 | 0.268 |    \- |
| brier_score_type_1 | Women |    0.184 | 0.171 | 0.198 |    \- |

\(a\) Type 1 recalibration

| name               | Model | estimate | lower | upper | p_val |
|:-------------------|:------|---------:|------:|------:|------:|
| brier_score_type_2 | Men   |    0.254 | 0.238 | 0.269 |    \- |
| brier_score_type_2 | Women |    0.184 | 0.173 | 0.199 |    \- |

\(b\) Type 2 recalibration

Table 7: Brier score results with recalibrated data
...
dplyr::bind_rows(
  MiceExtVal::get_forestplot_data(
    strat = "Men", type = "harrell_c_index", Framingham = men_model
    ),
  MiceExtVal::get_forestplot_data(
    strat = "Women", type = "harrell_c_index", Framingham = women_model
    )
) |>
...
| Framingham                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 0.69 (0.66, 0.71) | 0.71 (0.69, 0.74) | ![](data:image/svg+xml;base64,) |
|                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |                   |                   | ![](data:image/svg+xml;base64,)                                                                                                                                                                                                                                                                                                                         |
...
| Model                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Overall (95% CI)  |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Framingham Women                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | 0.71 (0.69, 0.74) | ![](data:image/svg+xml;base64,) |
| Framingham Men                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 0.69 (0.66, 0.71) | ![](data:image/svg+xml;base64,) |
|                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |                   | ![](data:image/svg+xml;base64,)             |
...
| Men                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | 0.26 (0.24, 0.27) | 0.25 (0.24, 0.27)             | 0.25 (0.24, 0.27)             | ![](data:image/svg+xml;base64,) |
| Women                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 0.18 (0.17, 0.20) | 0.18 (0.17, 0.20)             | 0.18 (0.17, 0.20)             | ![](data:image/svg+xml;base64,)         |
|                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |                   |                               |                               | ![](data:image/svg+xml;base64,)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
