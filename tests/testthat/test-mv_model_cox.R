...
test_that("Checks S0 properly", {
  expect_error(mv_model_cox(list(x = 0.08, z = 0.15), list(x = 1, z = 2), 2, "a"))
})

test_that("Model is generated properly MiceExtVal class", {
  model <- mv_model_cox(list(x = 0.08, z = 0.15), list(x = 1, z = 2), y ~ x + z, 0.8)
...
test_that("Coefficients are assigned properly", {
  model <- mv_model_cox(list(x = 0.08, z = 0.15), list(x = 1, z = 2), y ~ x + z, 0.8)
  expect_identical(model$coefficients, list(x = 0.08, z = 0.15))
})
