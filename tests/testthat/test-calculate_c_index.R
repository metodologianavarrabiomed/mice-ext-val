...
test_that("Returns an error if `formula` does not exist in `model`", {
  ...
  expect_error(calculate_c_index(model_cox_no_formula, data), "`model` must contain a valid `formula`")

  model_logreg_no_formula <- model_logreg
  model_logreg_no_formula$formula <- NULL
  expect_error(calculate_c_index(model_logreg_no_formula, data), "`model` must contain a valid `formula`")
})
...
test_that("Returns an error if `predictions_data` does not exist in `model`", {
  ...
  expect_error(calculate_c_index(model_cox_no_predictions_data, data), "`model` must contain `predictions_data` calculate it")

  model_logreg_no_predictions_data <- model_logreg
  model_logreg_no_predictions_data$predictions_data <- NULL
  expect_error(calculate_c_index(model_logreg_no_predictions_data, data), "`model` must contain `predictions_data` calculate it")
})
