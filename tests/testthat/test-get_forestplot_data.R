...
test_that("returns an error if some model is not <MiceExtVal>", {
  ...
  expect_error(get_forestplot_data(strat = "overall", type = "harrell_c_index", model_cox, 5), "must be <MiceExtVal>")
})
...
test_that("returns an error if some model does not have the `c_index` calculated", {
  ...
  expect_error(get_forestplot_data(strat = "overall", type = "harrell_c_index", model_cox, model_logreg), "must have their `harrell_c_index` calculated, consider using")
})
...
test_that("generates properly the forestplot data for `c_index`", {
  ...
  expect_s3_class(get_forestplot_data(strat = "overall", type = "harrell_c_index", model_cox, model_logreg), "tbl_df")
