...
Once we have defined our models the next step is to actually calculate their results. The `calculate_` functions allow us to recreate the highlighted part of the diagram. The statistical analysis must be performed in each of the imputed data sets separatelly and we aggregate the results afterwards.

![Flow diagram for the multiple imputation by chained equations methodology marking the results calculation part](man/figures/mermaid_diagram_2.svg)
...
![Flow diagram for the multiple imputation by chained equations methodology marking the visualization of the results part](man/figures/mermaid_diagram_3.svg)
